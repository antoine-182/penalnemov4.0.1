MODULE usrdef_istate
   ...
   !
   USE usrdef_nam, ONLY : rn_dz, rn_dam     ! User defined : namelist variables
   !
...
CONTAINS
   ...
   SUBROUTINE usr_def_istate( pdept, ptmask, pts, pu, pv, pssh )
      ...
      !
      IF(lwp) WRITE(numout,*)
      IF(lwp) WRITE(numout,*) 'usr_def_istate : SLOPE configuration, analytical definition of initial state'
      ...
      pssh(:,:)   = 0._wp
      !
      ! normalement <0 est maské
      !                          ! T & S profiles
      zdam = rn_dam                      ! density front position in kilometers
      pts(:,:,:,jp_tem) = 20._wp * ptmask(:,:,:)
      DO jk = 1, jpkm1
         ! WHERE( glamt(:,:) <= zdam )   pts(:,:,jk,jp_tem) = 10._wp * ptmask(:,:,jk)
         ! WHERE( glamt(:,:) <= zdam )   pts(:,:,jk,jp_tem) = 15._wp * ptmask(:,:,jk)
         ! WHERE( glamt(:,:) <= zdam .AND. pdept(:,:,jk) > 500._wp - rn_dz )   pts(:,:,jk,jp_tem) = 10._wp * ptmask(:,:,jk)   ! snake
         WHERE( glamt(:,:) <= 30._wp .AND. glamt(:,:) > 10._wp .AND. pdept(:,:,jk) < rn_dz )   pts(:,:,jk,jp_tem) = 10._wp * ptmask(:,:,jk)   ! instability static
         ! WHERE( glamt(:,:) <= 30._wp .AND. glamt(:,:) > 10._wp )   pts(:,:,jk,jp_tem) = 10._wp * ptmask(:,:,jk)   ! patch
      END DO
      !
