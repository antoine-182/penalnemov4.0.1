...
CONTAINS
   ...
   SUBROUTINE dia_wri( kt )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) ::   z3d,z3u,z3v   ! 3D workspace
      !!----------------------------------------------------------------------
      !
...
!!an
      IF ( iom_use("u_vol") ) THEN
        z3u  = 0._wp ; z3v = 0._wp
        DO ji=1,jpi
          DO jj=1,jpj
            DO jk=1,jpkm1
               z3u(ji,jj,jk) = un(ji,jj,jk) * e3u_n(ji,jj,jk) * e2u(ji,jj) * umask(ji,jj,jk)
               ! z3v(ji,jj,jk) = vn(ji,jj,jk) * e3v_n(ji,jj,jk) * e1v(ji,jj) * vmask(ji,jj,jk)
            END DO
          END DO
        END DO
        CALL iom_put( "u_vol", z3u )                ! volume transport U (include porosity)
        ! CALL iom_put( "v_vol", z3v )                ! volume transport V (include porosity)
      ENDIF
!!an
