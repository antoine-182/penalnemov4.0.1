MODULE usrdef_nam
   ...
   REAL(wp), PUBLIC ::   rn_x0     ! beginning of the slope [km]
   REAL(wp), PUBLIC ::   rn_dam    ! position of the front [km]
   REAL(wp), PUBLIC ::   rn_p      ! slope [m/km]
   !
...
CONTAINS
   ...
   SUBROUTINE usr_def_nam( cd_cfg, kk_cfg, kpi, kpj, kpk, kperio )
      ...
      !!
      NAMELIST/namusr_def/ ln_zco, ln_zps, ln_sco, rn_dx, rn_dz,rn_x0,rn_p, rn_dam,     &
...
902   IF( ios /= 0 )   CALL ctl_nam ( ios , 'namusr_def in configuration namelist' )
      ...
      !
      cd_cfg = 'SLOPE'           ! name & resolution (not used)
      kk_cfg = INT( rn_dx )
      !
      ! Global Domain size:  SLOPE domain is  200 km x 3 grid-points x 2000 m
      ...
      WRITE(numout,*) '~~~~~~~~~~~ '
      WRITE(numout,*) '   Namelist namusr_def : SLOPE test case'
      ...
      WRITE(numout,*) '      slope                                    rn_p   = ', rn_p, ' [m/km]'
      WRITE(numout,*) '      starting slope                           rn_x0  = ', rn_x0, ' [km]'
      WRITE(numout,*) '      beginning front                          rn_dam  = ', rn_dam, ' [km]'
      WRITE(numout,*) '      SLOPE domain = 200 km x 3 grid-points x 2000 m'
      WRITE(numout,*) '         resulting global domain size :        jpiglo = ', kpi
      WRITE(numout,*) '                                               jpjglo = ', kpj
      WRITE(numout,*) '                                               jpkglo = ', kpk
      !
      !                             ! Set the lateral boundary condition of the global domain
      kperio = 0                    ! SLOPE configuration : close basin (0), x periodic (1)
      !
      WRITE(numout,*) '   '
      WRITE(numout,*) '   Lateral boundary condition of the global domain'
      WRITE(numout,*) '      SLOPE : closed basin                  jperio = ', kperio
      !
#if defined key_bvp
