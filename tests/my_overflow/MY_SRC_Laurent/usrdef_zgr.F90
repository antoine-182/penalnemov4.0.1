...
CONTAINS
   ...
   SUBROUTINE usr_def_zgr( ld_zco  , ld_zps  , ld_sco  , ld_isfcav,    &   ! type of vertical coordinate
      ...
      !
      INTEGER  ::   ji, jj, jk,jl         ! dummy indices
      ...
      DO ji=1,jpi
        DO jj=1,jpj
          zht(ji,jj) = profilz(glamt(ji,jj),pdepw_1d)
        END DO
      END DO
      !
...
!!an
      ...
      !         1.1) with tanh
      ! zrpostar = rn_abp / (1._wp - rn_abp)
      ! z1d = 1 / ( 1._wp + zrpostar )
      ! DO ji = 1, jpi
      !   DO jj = 1, jpj
      !      DO jk = 1, jpk
      !        rpot(ji,jj,jk) = z1d * ( 0.5_wp*(1._wp+tanh( - ( pdept_1d(jk) - zht(ji,jj) ) / ( REAL(nn_cnp, wp) * rn_dz ) )) + zrpostar )
      !        rpou(ji,jj,jk) = z1d * ( 0.5_wp*(1._wp+tanh( - ( pdept_1d(jk) - zhu(ji,jj) ) / ( REAL(nn_cnp, wp) * rn_dz ) )) + zrpostar )
      !        rpow(ji,jj,jk) = z1d * ( 0.5_wp*(1._wp+tanh( - ( pdepw_1d(jk) - zht(ji,jj) ) / ( REAL(nn_cnp, wp) * rn_dz ) )) + zrpostar )
      !        !
      ...
      ! ! CALL lbc_lnk_multi( 'usrdef_zgr', rpot, 'T', 1.,  r1_rpot, 'T', 1.,     &
      ! !       &                           rpou, 'U', 1.,  r1_rpou, 'U', 1.,     &
      ! !       &                           rpow, 'W', 1.,  r1_rpow, 'W', 1.      )
      !
      !         1.2) with integrate zgr_pse
      !
      rpot(:,:,:) = rn_abp
      rpot(:,:,:) = 1._wp
      DO ji = 2, jpi
      ...
      DO jk=1,jpkm1
         DO ji=2,jpi
            if ((abs(rpot(ji,2,jk)-0.)>0.01).AND.(abs(rpot(ji,2,jk)-1)>0.01)) then
            print *,'POROSITE = ',ji,jk,rpot(ji,2,jk)
            STOP
            endif
         enddo
      enddo

      CALL lbc_lnk( 'usrdef_zgr', rpot, 'T', 1., kfillmode=jpfillnothing)
      !
      !! Shapiro filter (S=2/5)
      DO jl = 1, nn_bvpsmg
        z3d(:,:,:) = rpot(:,:,:)
        DO jk = 2,jpk-2
          DO ji = 2,jpim1
            DO jj = 2,jpjm1
              ! in degree
       !       IF ( (glamt(ji,jj) > 1._wp) .AND. (glamt(ji,jj) < 100._wp) ) THEN
                ! z3d(ji,jj,jk) = 0.1_wp * ( 6._wp * rpot(ji,jj,jk) + rpot(ji-1,jj,jk  ) + rpot(ji+1,jj,jk  )   &
                !   &                                               + rpot(ji  ,jj,jk-1) + rpot(ji  ,jj,jk+1)   )
                  !z3d(ji,jj,jk) = 0.2_wp * ( 3._wp * rpot(ji,jj,jk) + rpot(ji-1,jj,jk  ) + rpot(ji+1,jj,jk  ) )
                  z3d(ji,jj,jk) = 0.25_wp * ( 2._wp * rpot(ji,jj,jk) + rpot(ji-1,jj,jk  ) + rpot(ji+1,jj,jk  ) )
         !     ENDIF
            END DO
          END DO
        END DO
        CALL lbc_lnk( 'usrdef_zgr', z3d, 'T', 1._wp, kfillmode=jpfillnothing)
        rpot(:,:,:) = z3d(:,:,:)
        z3d(:,:,:) = rpot(:,:,:)
        DO jk = 2,jpk-2
          DO ji = 2,jpim1
       ...
       !       IF ( (glamt(ji,jj) > 1._wp) .AND. (glamt(ji,jj) < 100._wp) ) THEN
                ...
                !   &                                               + rpot(ji  ,jj,jk-1) + rpot(ji  ,jj,jk+1)   )
                  !z3d(ji,jj,jk) = 0.2_wp * ( 3._wp * rpot(ji,jj,jk) + rpot(ji-1,jj,jk  ) + rpot(ji+1,jj,jk  ) )
                  z3d(ji,jj,jk) = 0.25_wp * ( 2._wp * rpot(ji,jj,jk) + rpot(ji,jj,jk-1  ) + rpot(ji,jj,jk+1  ) )
         !     ENDIF
            END DO
          END DO
        END DO
      ...
      !
      ! to clean nil values
      WHERE( rpot(:,:,:) < rn_abp )   rpot(:,:,:) = rn_abp
      CALL lbc_lnk( 'usrdef_zgr', rpot, 'T', 1._wp, kfillmode=jpfillnothing)
      !
      !
      ...
      DO jk = 1, jpkm1
          DO jj = 1, jpj
            DO ji = 1, jpi
             ...
             rpow(ji,jj,jk) = MIN ( rpot(ji,jj,jk) , rpot(ji  ,jj,jk+1) )
           END DO
        END DO
      END DO
      ...
      !! no real need to call for communication, vertical decomposition
      CALL lbc_lnk_multi( 'usrdef_zgr', rpou, 'U', 1., rpow, 'W', 1., kfillmode=jpfillcopy)
      !
      DO ji = 1, jpi
        DO jj = 1, jpj
           DO jk = 1, jpk
...
!        DO jk = jpkm1, 1, -1
!           WHERE( rpot(:,:,jk) <= rn_abp )   k_bot(:,:) = (jk-1)*k_top(:,:)
!        END DO
  ...
  !      The computational grid is a flat bottom
  !      DO jk = jpkm1, 1, -1
...
!!an
   ...
   SUBROUTINE zgr_pse( ki, kj, kk,                  &
      ...
      zhA = profilz(zA(1),pdepth) ; zhC = profilz(zC(1),pdepth) ;  zhtt = profilz(plam(ki,kj),pdepth)
      !
      ...
      ELSE                            ! porous land
        ...
        DO ji = 1,nn_cnp
          zf1 = MIN(1._wp, MAX( 0._wp, (profilz(zxd,pdepth) - zC(2))/rn_dz) )
   ...
   FUNCTION profilz(x,pdepw)  RESULT(f)
      !!----------------------------------------------------------------------
      !!                 ***  ROUTINE profilz  ***
      !!
      ...
      IMPLICIT NONE
      REAL, INTENT(in) :: x
      REAL,DIMENSION(:),INTENT(in) :: pdepw
      REAL             :: f   ! result
      INTEGER :: jk, jkref
      REAL :: dist
      !!----------------------------------------------------------------------
      ...
      !find the closet w level
      !
      dist = huge(1._wp)
      DO jk=1,jpk
         if (abs(f-pdepw(jk))<dist) then
            jkref=jk
            dist = abs(f-pdepw(jk))
         endif
      ENDDO
