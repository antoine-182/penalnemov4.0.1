...
CONTAINS
   ...
   SUBROUTINE dyn_adv_ubs( kt )
...
#if defined key_ubs
      REAL(wp) ::   zfuk, zfvk
      REAL(wp), DIMENSION(jpi,jpj,jpk,2) ::   zlu_uw, zlv_vw
#endif
      !!----------------------------------------------------------------------
      !
      IF( kt == nit000 ) THEN
     ...
     DO jk = 2, jpkm1            !  Laplacian  !
       !                         ! =========== !
       !
         zlu_uw(:,:,jk,1) = ( ub(:,:,jk-1) - 2._wp*ub (:,:,jk) + ub (:,:,jk+1) ) * umask(:,:,jk)
         zlv_vw(:,:,jk,1) = ( vb(:,:,jk-1) - 2._wp*vb (:,:,jk) + vb (:,:,jk+1) ) * vmask(:,:,jk)
         !
         ! zlu_uw(:,:,jk,2) = ( zfu (:,:,jk+1) - 2.*zfu (:,:,jk) + zfu (:,:,jk-1) ) * umask(:,:,jk)
         ! zlv_vw(:,:,jk,2) = ( zfv (:,:,jk+1) - 2.*zfv (:,:,jk) + zfv (:,:,jk-1) ) * vmask(:,:,jk)
     END DO
