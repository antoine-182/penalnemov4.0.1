...
CONTAINS
   ...
   SUBROUTINE dia_wri( kt )
...
#if defined key_bvp
      IF( iom_use("rpot") )   CALL iom_put( "rpot", rpot(:,:,:) )
      IF( iom_use("rpou") )   CALL iom_put( "rpou", rpou(:,:,:) )
      IF( iom_use("bmpu") )   CALL iom_put( "bmpu", bmpu(:,:,:) )
      IF( iom_use("rpow") )   CALL iom_put( "rpow", rpow(:,:,:) )
#endif
      ...
      !
      IF ( iom_use("u_vol") ) THEN        ! 3D i-transport
        z3d(:,:,jpk) = 0._wp
        DO jk = 1, jpk
          z3d(:,:,jk) = e3u_n(:,:,jk) * e2u(:,:) * un(:,:,jk)
        END DO
        CALL iom_put( "u_vol", z3d(:,:,:) )          ! kinetic energy
      ENDIF
...
#if defined key_bvp
      CALL histdef( nid_U, "rpou", "U porosity"           , ""   ,   &  ! rpou
             &     jpi, jpj, nh_U, ipk, 1, ipk, nz_U, 32, clop, zsto, zout )
     CALL histdef( nid_U, "bmpu", "U friction"           , ""   ,   &  ! bmpu
            &     jpi, jpj, nh_U, ipk, 1, ipk, nz_U, 32, clop, zsto, zout )
#endif
        CALL histdef( nid_U, "utran", "Zonal Transport"                      , "m/s"    ,   &  ! un
           &          jpi, jpj, nh_U, ipk, 1, ipk, nz_U, 32, clop, zsto, zout )
...
!!an
#if defined key_bvp
      CALL histwrite( nid_T, "rpot", it, rpot            , ndim_T , ndex_T )    ! porotisity T
      CALL histwrite( nid_U, "rpou", it, rpou            , ndim_U , ndex_U )    ! porotisity T
      CALL histwrite( nid_U, "bmpu", it, bmpu            , ndim_U , ndex_U )    ! porotisity T
      CALL histwrite( nid_W, "rpow", it, rpow            , ndim_T , ndex_T )    ! porotisity T
#endif
      CALL histwrite( nid_U, "utran", it, un*e3u_n            , ndim_U , ndex_U )    ! i-current
!!an
...
#endif
      CALL iom_rstput( 0, 0, inum, "utran", un*e3u_n )    ! porotisity T
!!an
