...
CONTAINS
   ...
   SUBROUTINE tra_adv_fct( kt, kit000, cdtype, p2dt, pun, pvn, pwn,       &
      ...
      !!an ll_zAimp = T if wi != 0, need to partion
      IF( ll_zAimp ) THEN
         ...
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               ...
               !!an /e3t_a sera multiplié après par le volume
               !!an on traite implicitement wi avec un schema UP1
               !!an On prend que MAX/MIN garde que le w qui fait rentrer de la flotte dans la maille (en amont dans lecoulement)
...
!!an Upstream1 sur (u,v,wn) et pta+=trend(UP1) puis zwi first guess implicite (??) de pta
           ...
           DO jk = 1, jpkm1                 ! Horizontal advective fluxes
              DO jj = 1, jpjm1
                 DO ji = 1, fs_jpim1   ! vector opt.
                    ...
                    zfm_vj = pvn(ji,jj,jk) - ABS( pvn(ji,jj,jk) )
                   !
                    zC2t_u = pun(ji,jj,jk) * ( ptn(ji,jj,jk,jn) + ptn(ji+1,jj  ,jk,jn) )   ! 2 x C2 interpolation of T at u- & v-points
                    zC2t_v = pvn(ji,jj,jk) * ( ptn(ji,jj,jk,jn) + ptn(ji  ,jj+1,jk,jn) )
                    !                                                  ! C4 minus upstream advective fluxes
                    zwx(ji,jj,jk) =  0.5_wp * ( zC2t_u - zfp_ui * zltu(ji,jj,jk) - zfm_ui * zltu(ji+1,jj,jk) ) - zwx(ji,jj,jk)
                    zwy(ji,jj,jk) =  0.5_wp * ( zC2t_v - zfp_vj * zltv(ji,jj,jk) - zfm_vj * zltv(ji,jj+1,jk) ) - zwy(ji,jj,jk)
                 END DO
              END DO
           END DO
            !
        ...
        CASE(  3  )                   !- 3rd order upstream
          ...
          DO jk = 1, jpkm1
            ...
            z3d(:,:,jk+1) = ( ptn(:,:,jk,jn) - ptn(:,:,jk+1,jn) ) * wmask(:,:,jk+1)   ! W point
            ! Laplacian
            zltw(:,:,jk) = (  z3d(:,:,jk) - z3d(:,:,jk+1)  ) * r1_6     ! T point
          END DO
          ! no need to call lbc_lnk
          !
          DO jk = 2, jpkm1                 ! Vertical advective fluxes
             ! upstream scheme
             zfp_wi = pwn(:,:,jk) + ABS( pwn(:,:,jk) )
             zfm_wi = pwn(:,:,jk) - ABS( pwn(:,:,jk) )
            !
             zC2t_w = pwn(:,:,jk) * ( ptn(:,:,jk,jn) + ptn(:,:,jk-1,jn) )   ! 2 x C2 interpolation at W
             !                                                  ! C4 minus upstream advective fluxes
             zwz(:,:,jk) =  ( 0.5_wp * ( zC2t_w - zfp_wi * zltw(:,:,jk  )                   &
                &                               - zfm_wi * zltw(:,:,jk-1) ) - zwz(:,:,jk)   ) * wmask(:,:,jk)
          END DO
           !
         ...
         ENDIF
         !
!!an build the antidiffusive flux with wi
         IF ( ll_zAimp ) THEN
            DO jk = 1, jpkm1     !* trend and after field with monotonic scheme
               DO jj = 2, jpjm1
                  DO ji = fs_2, fs_jpim1   ! vector opt.
...
!!an ztw is first guess temp +
   ...
   SUBROUTINE tra_adv_up1( kt, kit000, cdtype, p2dt, pun, pvn, pwn,       &
      ...
      REAL(wp), DIMENSION(jpi,jpj)            ::   zfp_wi, zfm_wi
      REAL(wp), DIMENSION(jpi,jpj,jpk)        ::   zwx, zwy, zwz, ztu, ztv, z3d
      REAL(wp), DIMENSION(:,:,:), ALLOCATABLE ::   ztrdx, ztrdy, ztrdz, zptry
      !!----------------------------------------------------------------------
      !
      IF( kt == kit000 )  THEN
