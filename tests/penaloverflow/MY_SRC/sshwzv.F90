...
CONTAINS
   ...
   SUBROUTINE ssh_nxt( kt )
      ...
      !
!!an ssh results from the sum form bot to top of penalised divergence
!!   the elevation of ssh comes from the sum from bottom of vertical transport
      ...
      ELSE   ! z_star and linear free surface cases
        ...
        ! avec penalisation w peut devenir une vitesse penalisee
         DO jk = jpkm1, 1, -1                       ! integrate from the bottom the hor. divergence
            ...
            wn(:,:,jk) = r1_rpow(:,:,jk) * ( wn(:,:,jk+1) * rpow(:,:,jk+1)                    &
               &                             -  (  e3t_n(:,:,jk) * hdivn(:,:,jk)              &
               &                                + z1_2dt * ( e3t_a(:,:,jk) - e3t_b(:,:,jk) )  ) )  * tmask(:,:,jk)
         END DO
#else
   ...
   SUBROUTINE wAimp( kt )
      ...
      REAL(wp) , PARAMETER ::   Cu_min = 0.15_wp                      ! local parameters ! =0.15
      REAL(wp) , PARAMETER ::   Cu_max = 0.30_wp                      ! local parameters ! =0.30
      REAL(wp) , PARAMETER ::   Cu_cut = 2._wp*Cu_max - Cu_min        ! local parameters ! =0.45
...
!                  ENDIF
                  !
                  !!an CU_min : condition CFL à ne pas dépasser
                  ...
                  ENDIF
                  zcff = MIN(1._wp, zcff)
                  !!an full implicit (mais du coup plus diffusif)
                  ! zcff = 1._wp
                  !!an
