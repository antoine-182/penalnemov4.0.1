               ...
               DO jk = 1, jpkm1
                  DO jj = 2, jpjm1
                     DO ji = fs_2, fs_jpim1   ! vector opt. (ensure same order of calculation as below if wi=0.)
                        ...
                        zwi(ji,jj,jk) = zzwi + p2dt *   MIN( rpow(ji,jj,jk  ) * wi(ji,jj,jk  ) , 0._wp )
                        zws(ji,jj,jk) = zzws - p2dt *   MAX( rpow(ji,jj,jk+1) * wi(ji,jj,jk+1) , 0._wp )
#else
               ...
               DO jk = 1, jpkm1
                  DO jj = 2, jpjm1
                     DO ji = fs_2, fs_jpim1   ! vector opt.
                        ...
                        zwd(ji,jj,jk) = e3t_a(ji,jj,jk) - zwi(ji,jj,jk) - zws(ji,jj,jk)   !!an RHS(T)=0
                    END DO
                  END DO
               END DO
            ENDIF
