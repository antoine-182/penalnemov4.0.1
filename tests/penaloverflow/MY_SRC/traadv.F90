...
!!an
      ...
      ENDIF
!!an
      IF( ln_traadv_fct .AND. ( nn_fct_h /= 2 .AND. nn_fct_h /= 4 .AND. nn_fct_h /= 3 )   &          ! FCT
                        .AND. ( nn_fct_v /= 2 .AND. nn_fct_v /= 4 .AND. nn_fct_v /= 3 )   ) THEN
        CALL ctl_stop( 'tra_adv_init: FCT scheme, choose 2nd, 3rd or 4th order' )
!!an
      ENDIF
