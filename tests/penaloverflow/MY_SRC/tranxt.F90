...
CONTAINS
   ...
   SUBROUTINE tra_nxt( kt )
      ...
      ELSE                                            ! Leap-Frog + Asselin filter time stepping
         !
#if defined key_bvp
        ! Time stepping + Asselin : does not need actually the volume (linear in time ! )
        ! IF( ln_linssh ) THEN   ;   CALL tra_nxt_vvl( kt, nit000, rdt, 'TRA', tsb, tsn, tsa,   &
        !    &                                                                sbc_tsc, sbc_tsc_b, jpts )
