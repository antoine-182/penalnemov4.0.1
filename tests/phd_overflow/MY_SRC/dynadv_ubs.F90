...
CONTAINS
   ...
   SUBROUTINE dyn_adv_ubs( kt )
      ...
      IF( kt == nit000 ) THEN
         IF(lwp) WRITE(numout,*)
         IF(lwp) WRITE(numout,*) 'dyn_adv_ubs : UBS flux form momentum advection on horizontal and C2 vertical'
         IF(lwp) WRITE(numout,*) '~~~~~~~~~~~'
      ENDIF
      !
      zfu_t(:,:,:) = 0._wp
      ...
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1
            zfu_uw(ji,jj,1) = 0.5_wp * ( e1e2t(ji  ,jj) * wn(ji  ,jj,1) * rpow(ji  ,jj,1)   &
               &                       + e1e2t(ji+1,jj) * wn(ji+1,jj,1) * rpow(ji+1,jj,1)   ) * un(ji,jj,1)
            zfv_vw(ji,jj,1) = 0.5_wp * ( e1e2t(ji,jj  ) * wn(ji,jj  ,1) * rpow(ji,jj  ,1)   &
               &                       + e1e2t(ji,jj+1) * wn(ji,jj+1,1) * rpow(ji,jj+1,1)   ) * vn(ji,jj,1)
         END DO
      END DO
      ENDIF
      !
      ...
      !
#if defined key_ubs
      ! define a real UBS on the vertical
      DO jk = 1, jpk
         !                                         ! horizontal volume fluxes
         zfu(:,:,jk) = e2u(:,:) * e3u_n(:,:,jk) * un(:,:,jk)
         zfv(:,:,jk) = e1v(:,:) * e3v_n(:,:,jk) * vn(:,:,jk)
     ...
     !! init to 0
     !! a jpkm1 pas sur
     ! zlu_uw(:,:,jpkm1,1) = 0._wp
     ! zlv_vw(:,:,jpkm1,1) = 0._wp
   ...
   SUBROUTINE dyn_adv_up3( kt )
      ...
      DO jk = 1, jpkm1                       ! ====================== !
         !                                         ! horizontal volume fluxes
         zfu(:,:,jk) = 0.25_wp * e2u(:,:) * e3u_n(:,:,jk) * un(:,:,jk)  !!an bvp: already in e3u
         zfv(:,:,jk) = 0.25_wp * e1v(:,:) * e3v_n(:,:,jk) * vn(:,:,jk)
         !
         DO jj = 1, jpjm1                          ! horizontal momentum fluxes at T- and F-point
            DO ji = 1, fs_jpim1   ! vector opt.
               ...
               ENDIF
               !
               zfv_f(ji  ,jj  ,jk) = zfvi * ( un(ji,jj,jk) + un(ji  ,jj+1,jk) - gamma1 * zl_u )
               zfu_f(ji  ,jj  ,jk) = zfuj * ( vn(ji,jj,jk) + vn(ji+1,jj  ,jk) - gamma1 * zl_v )
            END DO
         END DO
         DO jj = 2, jpjm1                          ! divergence of horizontal momentum fluxes
            DO ji = fs_2, fs_jpim1   ! vector opt.
