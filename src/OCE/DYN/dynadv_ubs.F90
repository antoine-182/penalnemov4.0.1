...
CONTAINS
   ...
   SUBROUTINE dyn_adv_ubs( kt )
      ...
      !!an wn est la partie explicite de w, la partie implicite est advecté via la diffusion verticale
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1
...
! #if defined key_bvp
!                zfw(ji,jj,jk) = 0.25_wp * e1e2t(ji,jj) * wn(ji,jj,jk) * rpow(ji,jj,jk)   !!an to me
! #else
!                zfw(ji,jj,jk) = 0.25_wp * e1e2t(ji,jj) * wn(ji,jj,jk)
! #endif
            END DO
